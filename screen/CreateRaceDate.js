...
export default class CreateRaceDate extends Component {
    ...
    navigateControl = (ev) => {
        ...
        const start_date = this.state.year+ "-" + (this.state.month> 9 ? this.state.month : '0'+this.state.month) + "-" +(this.state.date> 9 ? this.state.date : '0'+this.state.date);
        const start_time = (this.state.hour> 9 ? this.state.hour : '0'+this.state.hour)+ ":" + (this.state.minute> 9 ? this.state.minute : '0'+this.state.minute);
        if(racetype == true){//check if race is flextime or not
            this.props.navigation.navigate('CreateDateEnd',{race: racetype,title: title,distance:distance,startdate:start_date,starttime:start_time})
        }else{
            const headers_conf = {
            ...
            console.log(params);
              axios.post(
                'http://103.253.25.177/rest_server/api/Race', params,headers_conf).then(function (response) {   
        ...
        return (
            <Container>
                ...
                <View
                    ...
                    <Text style={{color:'black'}}>Set Start Date</Text>
                ...
                <View
                    ...
                    <Text style={{color:'white'}}>Set Race Start Time</Text>
